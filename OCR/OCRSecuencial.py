...
images_dir = 'D:/Escritorio/tfgdataset/PRUEBADATASETS/imagesval' 
...
def MostrarDataset():
    ...
    for Cantidad in range(batch_size):
        print(Cantidad)
        batch = next(iter(val_dataloader))
        images,labels = batch
        images=yolo_resize_transform(images) # resize la imagen 
        images = images[Cantidad].permute(1,2,0).numpy() #aqui esta  la imagen y depende el numero segun el batch size

        model = torch.hub.load('D:/Escritorio/Prueba/yolov5-master', 'custom', path='bestCustomServerExp13.pt',force_reload=True,source='local')
        classes = model.names
